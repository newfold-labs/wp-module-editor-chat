...
export async function handleEditLogo(toolCall, args, ctx) {
	await ctx.updateProgress(__("Editing logo…", "wp-module-editor-chat"), 500);
	try {
		const mcpResult = await callAbility(ctx.mcpClient, "blu-edit-logo", args);
		const result = {
			id: toolCall.id,
